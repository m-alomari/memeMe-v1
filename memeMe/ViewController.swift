...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        self._configurations(comment: topComment, defaultText: "TOP")
        self._configurations(comment: bottomComment, defaultText: "BOTTOM")
    ...
    @IBAction func share(_ sender: Any)
    {
        ...
        let meme = Meme(topComment: topComment.text!, bottomComment: bottomComment.text!, originalImage: imageView.image!, memedImage: memedImage)
        
        let activityViewController = UIActivityViewController(activityItems: [meme.memedImage] , applicationActivities: nil)
        activityViewController.popoverPresentationController?.sourceView = self.view
        activityViewController.completionWithItemsHandler = {(activityType: UIActivity.ActivityType?, completed: Bool, returnedItems: [Any]?, error: Error?) in
            if !completed {
                print(debugger[20] ?? nil!) // HERE
                return
            }
            print(debugger[21] ?? nil!) // HERE
        }
